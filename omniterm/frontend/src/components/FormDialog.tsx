...
interface Props {
  title: string
  fields: { label: string; value: string; set: (v: string) => void; placeholder?: string; type?: string; multiline?: boolean }[]
  onConfirm: () => void
  onCancel: () => void
  confirmLabel?: string
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onCancel}>
      <div className="bg-vscode-panel border border-vscode-border rounded-lg shadow-2xl w-80" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-3 space-y-2">
          {fields.map(f => (
            <div key={f.label}>
              <label className="block text-[10px] text-vscode-text-dim mb-0.5">{f.label}</label>
              {f.multiline ? (

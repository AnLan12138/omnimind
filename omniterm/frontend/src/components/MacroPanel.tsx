...
export default function MacroPanel({ searchTerm = '', onClose, onSendMacro }: Props) {
  ...
  const deleteCmd = (id: string) => { setCmds(cmds.filter(c => c.id !== id)); setCtx(null) }
          ...
          <>
            {items.map(c => (
              <div key={c.id} draggable onDragStart={e => handleDragStart(e, c.id)}
                ...
                <button onClick={() => deleteCmd(c.id)} className="p-0.5 hover:bg-vscode-hover rounded opacity-0 group-hover/item:opacity-100"><Trash2 size={10} className="text-vscode-text-muted hover:text-vscode-red" /></button>
              </div>
            ))}
        ...
        <div className="fixed z-50 w-44 bg-vscode-input border border-vscode-border shadow-xl py-0.5" style={{ left: ctx.x, top: ctx.y }}>
          {ctx.type === 'cmd' && ctx.cmd && (
            <>
              ...
              <button onClick={() => deleteCmd(ctx.cmd!.id)} className="w-full flex items-center gap-2 px-3 py-1 hover:bg-vscode-hover text-[12px] text-vscode-red"><Trash2 size={12} /> 删除</button>
            </>
          )}
